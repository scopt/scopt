...
By default, scopt assumes you are using this library to parse your stand-alone program's command-line arguments.  In another context, however, you may want scopt to process arguments as a feature of your program, for example if you were writing something like an "operating system" that handled commands. In this case, you don't want scopt to output things to stdout or stderr, or to automatically terminate your process.

If you wish to hold output, instead of creating scopt.OptionParser, create scopt.OptionParserHoldOutput instead.  When using OptionParserHoldOutput, no output of any kind will be generated by scopt.  If you wish to access the strings of text that would have gone out to stderr or stdout, call parser.getErr() or parser.getOut() respectively.

Note that the parser object stores these strings internally.  Each run of the parser eliminates the old values and refreshes with new strings.  Also, these held output strings are not thread-safe if the parser is shared between threads!  (This is also why this feature wasn't implemented by simply redirecting stdout and stderr to strings.)  If you have one parser per thread, you should be fine.
